...
class BQ_BaseBBOS(BQ_SSR):
    def __init__(self, hall: Hall):
        ...
        self.bet_time_df: pd.DataFrame() = pd.DataFrame()
        self.cash_df: pd.DataFrame() = pd.DataFrame()
        self.online_month_game_df: pd.DataFrame() = pd.DataFrame()
        self.promotion_type_df: pd.DataFrame() = pd.DataFrame()

    def get_init_bet_time(self):  # 首次取得最後15日下注資料
        if path.exists(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_bet_time_15days.csv'):
        ...
        try:
            self.statement = (
                ...
                EXTRACT(HOUR FROM bet.`at`) AS bet_hour, 
                DATE(bet.`at`) AS data_date, 
                EXTRACT(DAYOFWEEK FROM bet.`at`) AS data_weekday, 
                SUM(bet.valid_bet) AS commissionable 
                FROM `{self.project_id}.ssr_dw_{self.domain_name}.Wager_View` bet 
                JOIN `{self.project_id}.ssr_dw_{self.domain_name}.user_info` member 
                ON bet.user_id = member.user_id 
                WHERE DATE(bet.`at`) <= '{self.data_date}' 
                GROUP BY 
                bet.user_id, 
                member.username, 
                member.created_at, 
                EXTRACT(HOUR FROM bet.`at`), 
                DATE(bet.`at`), 
                EXTRACT(DAYOFWEEK FROM bet.`at`) 
                """
            )
            self.exec()
            ...
            result_df.to_csv(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_bet_time_15days.csv', index=False)
    ...
    def get_latest_bet_time(self):  # 將檔案concat最新一日下注資料，重新取得15天實動檔案
        try:
            self.bet_time_df = pd.read_csv(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_bet_time_15days.csv')
            ...
            activate_bet_df = activate_bet_df[
                activate_bet_df['data_date'] >= activate_bet_df['top_activate_date']].reset_index(drop=True)
            activate_bet_df.to_csv(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_bet_time_15days.csv', index=False)
    ...
    def get_init_cash(self):  # 首次取得最後15日現金資料
        if path.exists(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_cash_15days.csv'):
        ...
        try:
            self.statement = (
                ...
                DATE_ADD(member.created_at, INTERVAL 12 HOUR) AS register_date, 
                opcode, 
                DATE(cash.created_at) AS data_date, 
                SUM(amount) AS amount 
                FROM `{self.project_id}.ssr_dw_{self.domain_name}.Cash_Entry_View` cash 
                JOIN `{self.project_id}.ssr_dw_{self.domain_name}.user_info` member 
                ON cash.user_id = member.user_id 
                WHERE DATE(cash.created_at) <= '{self.data_date}' 
            ...
            result_df.to_csv(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_cash_15days.csv', index=False)
    ...
    def get_latest_cash(self):  # 將檔案concat最新一日下注資料，重新取得15天實動檔案
        try:
            self.cash_df = pd.read_csv(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_cash_15days.csv')
            cash_file_df = df_type_format(self.cash_df)
            ...
            cash_df = df_type_format(cash_df)
            ...
            activate_cash_df = activate_cash_df[
                activate_cash_df['data_date'] >= activate_cash_df['top_activate_date']].reset_index(drop=True)
            activate_cash_df.to_csv(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_cash_15days.csv', index=False)
            self.cash_df = pd.read_csv(f'{Environment.ROOT_PATH}/files/spanner/{self.domain_name}_cash_15days.csv')
        except Exception as e:
            raise

    def get_online_month_game(self):
        try:
            self.mode = self.QUERY_MODE
            self.statement = (
                f"""
                SELECT vendor_id, kind, game_code, enable 
                FROM `{self.project_id}.general_information.game_code_dict` 
                WHERE enable = 1 AND 
                DATE(online_at) <= DATE_ADD(current_date(), INTERVAL -30 DAY)
                """
            )
            self.exec()
            self.online_month_game_df = self.fetch_data
    ...
    def get_promotion_type(self):
        try:
            ...
            self.statement = (
                f"""
                SELECT DISTINCT promotion.domain_id, promotion.promotion_id, type.type, type.name 
                FROM `{self.project_id}.bbos_data_uxm.Promotion` promotion 
                JOIN (
                  SELECT DISTINCT type, name 
                  FROM `{self.project_id}.bbos_data_uxm.Promotion_Type`
                  ) type 
                ON promotion.promotion_type = type.type
                WHERE domain_id = {self.domain_id}
                ORDER BY promotion_id
                """
            )
            self.exec()
            self.promotion_type_df = self.fetch_data
