...
class ETL_Spanner_BBOS_Demo_Service(CDP_BBOS):
    ...
    def member_info(self, hall: Hall, start, end):
        ...
        # CONCAT(SUBSTR(username, 0, 3), '*****') AS user_name,
        self.bq_db.statement = (
            ...
            email_confirm,
            phone_confirm
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.user_info`  
            WHERE
    ...
    def login_log(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            FROM (
                ...
                FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.login_log` log 
                JOIN `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.User_View` member 
                    ON log.user_id = member.user_id 
            LEFT JOIN (
                SELECT user_id, 'App' AS host, data_date 
                FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.login_log` 
    ...
    def bet_analysis(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            FROM
              `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.bet_analysis_jackpot` bet
            JOIN
              `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.user_info` member
            ON
    ...
    def deposit_withdraw(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            FROM ( 
                ...
                data_date, financial_year , financial_month , financial_week 
                FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.deposit_record`
                    WHERE status <> 0 
                    GROUP BY user_id, data_date, financial_year , financial_month , financial_week 
            ) deposit 
            JOIN `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.user_info` member 
            ...
            LEFT JOIN ( 
                ...
                data_date 
                FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.withdraw_record` 
                    WHERE status <> 0 
                    GROUP BY user_id, data_date 
    ...
    def offer(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            data_date, financial_year, financial_month, financial_week, 
            member.level_id AS user_level_id, member.level_name AS user_level 
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.offer_info` offer 
            JOIN `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.user_info` member 
    ...
    def dispatch(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            data_date, financial_year, financial_month, financial_week 
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.offer_dispatch` dispatch 
    ...
    def applicant(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.offer_applicant` applicant 
            JOIN `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.offer_promotion_info` promotion 
                ON applicant.promotion_id = promotion.promotion_id 
            LEFT JOIN `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.offer_dispatch` info 
    ...
    def promotion(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.offer_promotion_info` 
    ...
    def profit_loss(self, hall: Hall, start, end):
        ...
        self.bq_db.statement = (
            ...
            data_date, financial_year, financial_month, financial_week 
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.bet_analysis_jackpot` bet 
            WHERE data_date BETWEEN '{start}' AND '{end}' 
    ...
    def vip_level(self, hall: Hall):
        ...
        self.bq_db.statement = (
            ...
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.User_View` member 
            WHERE member.role = 1
            """
        )
        self.bq_db.mode = self.bq_db.QUERY_MODE
        ...
        bq_gh.statement = (
            ...
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.user_vip_level` user_vip_level 
            JOIN `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.vip_level` vip_level 
            ON user_vip_level.vip_id = vip_level.vip_id 
            WHERE 
    ...
    def vip_login(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.Login_View` login 
            JOIN `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.User_View` member 
            ON login.user_id = member.user_id 
            LEFT JOIN `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.user_vip_level` vip 
    ...
    def login_location(self, hall: Hall, start, end):
        self.bq_db.statement = (
            ...
            ( 
                ...
                FROM `{self.bq_db.project_id}.ssr_dw_demo_{hall.domain_name}.Login_View` 
