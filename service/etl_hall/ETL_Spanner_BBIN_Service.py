...
def try_exc_func(func):
    def wrap_func(*args, **kwargs):
        ...
        try:
            ...
            notify_bot.message = (
                f"CDP ETL Spanner: {kwargs['hall'].hall_name}, {func.__name__} error\n"
                f"Error Message: {e} "
            )
            ...
            print(f"Insert into cdp-instance {kwargs['hall'].hall_name} {func.__name__} exec time: {result_time[0]} minutes, {result_time[1]} seconds")

    return wrap_func


class ETL_Spanner_BBIN_Service(CDP_BBIN):
    def __init__(self, spanner_db: str, bq_db):
        super(ETL_Spanner_BBIN_Service, self).__init__(database=spanner_db)
        self.bq_db = bq_db
